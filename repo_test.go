...
func TestCloneFromSSHShouldFailed(t *testing.T) {
	...
	_, err := Clone(path, "git@github.com:fsamin/go-repo.git", WithSSHAuth(testRSAKey), WithVerbose(nil))
	assert.Error(t, err)
}
...
func TestCloneFromSSHShouldSuccess(t *testing.T) {
	...
	_, err = Clone(path, "git@github.com:fsamin/go-repo.git", WithSSHAuth(pkey), WithVerbose(nil))
	require.NoError(t, err)
}
...
func TestCloneFromHTTPShouldSuccess(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(path, os.FileMode(0755)))
	_, err := Clone(path, "https://github.com/fsamin/go-repo.git", WithHTTPAuth("fsamin", os.Getenv("TEST_TOKEN")), WithVerbose(nil))
	require.NoError(t, err)
}
